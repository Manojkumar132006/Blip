...
class routineSchema(BaseModel):
    ...
    start_time: str = Field(default_factory=lambda: datetime.datetime.now(
    ).isoformat())  # only time so as to apply to all days
    ...
    # Reference to the group this routine belongs to
...
class sparkSchema(BaseModel):
    ...
    start_time: str = Field(
        default_factory=lambda: datetime.datetime.now().isoformat())  # including date
    ...
    # Reference to the group this spark belongs to
    group: Optional[str] = Field(default=None)
